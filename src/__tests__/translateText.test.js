...
jest.mock('react-native-nitro-modules', () => ({
    NitroModules: {
        createHybridObject: jest.fn((name) => name === 'TextRecognizer' ? mockRecognizer : mockTranslator),
    },
}));
...
describe('createTranslatorPlugin', () => {
    beforeEach(() => {
        ...
        NitroModules.createHybridObject.mockImplementation((name) => name === 'TextRecognizer' ? mockRecognizer : mockTranslator);
    });
    ...
    describe('translate function', () => {
        ...
        it('returns empty string for undefined/null-like input', async () => {
            ...
            const result = await handle.translate(undefined);
            expect(result).toBe('');
        });
    });
});
