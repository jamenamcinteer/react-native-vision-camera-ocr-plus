...
export const Camera = forwardRef(function Camera(props, ref) {
    ...
    const recognizerHandle = useTextRecognition(mode === 'recognize' ? options : undefined);
    const translatorHandle = useTranslate(mode === 'translate' ? options : undefined);
    ...
    const recognizer = mode === 'translate'
    ...
    const runTranslate = useMemo(() => runOnJS((text) => {
        if (!text)
            return;
        translator
    ...
    return (React.createElement(React.Fragment, null, !!device && (React.createElement(NativeCamera, { pixelFormat: "yuv", ref: ref, device: device, frameProcessor: frameProcessor, ...p }))));
});
/**
